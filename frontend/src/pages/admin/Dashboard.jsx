...
import { profileAPI, skillsAPI, projectsAPI, messagesAPI, githubAPI } from '../../services/api';
  ...
  const loadStats = async () => {
    try {
      ...
      console.log('Dashboard Stats:', {
        ...
        github: results[3].data,
        profile: results[4].data
      });

      setStats({
        skills: results[0].data?.length || 0,
        projects: results[1].data?.length || 0,
        messages: results[2].data?.length || 0,
        githubStats: results[3].data,
      });
      
      // Get recent projects (latest 3)
      setRecentProjects(results[1].data?.slice(0, 3) || []);
      
      // Get profile
      if (results[4].data) {
        setProfile(results[4].data);
      }
    } catch (err) {
